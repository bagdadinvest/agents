(function () {
  function initCarousel(root) {
    ...
    var originals = items.filter(function (li) { return !li.classList.contains('__cloned'); });
    if (originals.length && originals.length !== items.length) {
      var ul = document.createElement('ul');
      ul.className = 'alice-carousel__stage';
      originals.forEach(function (li) { ul.appendChild(li); });
    ...
    function next() { index++; clampIndex(); applyTransform(); }
    function prev() { index--; clampIndex(); applyTransform(); }

    function startAuto() {
      stopAuto();
      autoTimer = setInterval(function () { index++; clampIndex(); applyTransform(); }, 3000);
    }
    ...
    wrapper.addEventListener('touchstart', function (e) {
      ...
      startX = e.touches[0].clientX; deltaX = 0; swiping = true; stopAuto();
    }, { passive: true });
    ...
    wrapper.addEventListener('touchend', function () {
      ...
      var threshold = 40; // px
      if (deltaX > threshold) prev(); else if (deltaX < -threshold) next();
      startAuto();
    });
